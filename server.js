...
const ENV = {
  ...
  // Polymarket APIs (ציבורי - לא צריך key)
  ...
  // News - GNews API
  NEWS_API_KEY:    process.env.NEWS_API_KEY    || 'd16661041a35be8695c67ac698823966',
  // AI - Groq (llama3, חינם)
  GROQ_API_KEY:    process.env.GROQ_API_KEY    || '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',
  // Alerts
  TELEGRAM_TOKEN:  process.env.TELEGRAM_TOKEN  || '',
  TELEGRAM_CHAT:   process.env.TELEGRAM_CHAT_ID|| '',
  // Blockchain - Alchemy Polygon
  POLYGON_RPC:     process.env.POLYGON_RPC     || 'https://polygon-mainnet.g.alchemy.com/v2/IqdmsVoHxofbVBhrOkPWH',
...
async function fetchMarkets() {
  try {
    const res = await axios.get(ENV.GAMMA_API_URL + '/markets', {
      params: {
        active:    true,
        closed:    false,
        limit:     100,
...
async function fetchNews() {
  ...
  try {
    const res = await axios.get('https://gnews.io/api/v4/search', {
      params: {
        q:       'Bitcoin OR Trump OR Federal Reserve OR Ukraine OR Polymarket',
        lang:    'en',
        country: 'us',
        max:     10,
...
async function fetchWallets() {
  try {
    console.log('[WALLETS] Starting ultimate wallet scan...');

    // ── שלב 1: שאב top 500 traders מ-3 leaderboards שונים ──
    const [profitBoard, roiBoard, volumeBoard] = await Promise.allSettled([
      axios.get(ENV.DATA_API_URL + '/leaderboard', {
        params: { limit: 100, offset: 0, order: 'PROFIT', filter: 'ALL' },
        timeout: 12000,
      }),
      axios.get(ENV.DATA_API_URL + '/leaderboard', {
        params: { limit: 100, offset: 0, order: 'PROFIT_PERCENTAGE', filter: 'ALL' },
        timeout: 12000,
      }),
      axios.get(ENV.DATA_API_URL + '/leaderboard', {
        params: { limit: 100, offset: 100, order: 'PROFIT', filter: 'ALL' },
        timeout: 12000,
      }),
    ]);

    // מיזוג כל הארנקים ללא כפולות
    const walletMap = new Map();

    const processBoard = (result) => {
      if (result.status !== 'fulfilled') return;
      const traders = result.value.data?.data || result.value.data || [];
      if (!Array.isArray(traders)) return;
      traders.forEach(t => {
        const addr = (t.proxyWallet || t.address || t.id || '').toLowerCase();
        ...
        if (!walletMap.has(addr)) {
          walletMap.set(addr, {
            ...
            volume:       parseFloat(t.volume || t.collateralVolume || 0),
            trades:       parseInt(t.tradesCount || t.numTrades || 0),
            wins:         parseInt(t.positiveROITrades || t.profitableTrades || 0),
            roi:          parseFloat(t.profitPercentage || t.roi || 0),
            winRate:      0,
          });
        }
      });
    ...
    [profitBoard, roiBoard, volumeBoard].forEach(processBoard);
    console.log('[WALLETS] Raw traders collected:', walletMap.size);

    // ── שלב 2: חשב win rate מדויק לכל trader ──
    const traders = Array.from(walletMap.values());

    // עבד עד 50 traders המובילים לקבלת נתונים מפורטים
    const topTraders = traders
      .filter(t => t.profit > 0 || t.trades > 10)
      .sort((a, b) => b.profit - a.profit)
      .slice(0, 50);

    // שאב פרטים מ-activity API בקבוצות של 5 במקביל
    const batchSize = 5;
    for (let i = 0; i < Math.min(topTraders.length, 30); i += batchSize) {
      const batch = topTraders.slice(i, i + batchSize);
      await Promise.allSettled(batch.map(async (trader) => {
        try {
          const res = await axios.get(ENV.DATA_API_URL + '/activity', {
            params: { user: trader.address, limit: 100 },
            timeout: 6000,
          });
          const activity = res.data?.data || res.data || [];
          if (Array.isArray(activity) && activity.length) {
            const positions = activity.filter(a => a.type === 'TRADE' || a.asset);
            const wins = positions.filter(a => parseFloat(a.pnl || a.profit || 0) > 0).length;
            if (positions.length > 0) {
              trader.trades   = positions.length;
              trader.wins     = wins;
              trader.winRate  = Math.round((wins / positions.length) * 100);
              // חשב ROI מדויק
              const totalIn = positions.reduce((s, a) => s + parseFloat(a.amount || a.size || 0), 0);
              if (totalIn > 0 && trader.profit) {
                trader.roi = Math.round((trader.profit / totalIn) * 100);
              }
              // חשב avg position size
              trader.avgPosition = totalIn > 0 ? Math.round(totalIn / positions.length) : 0;
              // streak: כמה ניצחונות ברצף אחרוני
              let streak = 0;
              for (let j = positions.length - 1; j >= 0; j--) {
                if (parseFloat(positions[j].pnl || positions[j].profit || 0) > 0) streak++;
                else break;
              }
    ...
    // ── שלב 3: סינון חכם - רק שחקנים שווים לעקוב ──
    const smartWallets = traders
      .map(t => {
        // חשב win rate אם אין
        if (!t.winRate && t.trades > 0 && t.wins > 0) {
          t.winRate = Math.round((t.wins / t.trades) * 100);
        }
        // חשב ROI אם אין
        if (!t.roi && t.volume > 0 && t.profit) {
          t.roi = Math.round((t.profit / t.volume) * 100);
        }
        // ציון כולל - Smart Score
        let score = 0;
        if (t.roi > 500)   score += 40;
        else if (t.roi > 200) score += 25;
        else if (t.roi > 100) score += 15;
        else if (t.roi > 50)  score += 8;

        if (t.winRate > 80) score += 30;
        else if (t.winRate > 70) score += 20;
        else if (t.winRate > 60) score += 12;

        if (t.profit > 100000) score += 20;
        else if (t.profit > 50000) score += 15;
        else if (t.profit > 10000) score += 8;

        if (t.trades > 100) score += 10;
        else if (t.trades > 50) score += 6;
        else if (t.trades > 20) score += 3;

        t.smartScore = score;
        t.isWhale    = t.profit > 50000 || (t.avgPosition && t.avgPosition > 5000);
        t.isSmart    = score >= 60;

        return t;
      })
      .filter(t =>
        t.profit > 1000 &&    // רק רווח אמיתי
        t.trades > 5          // לא מקרי
      )
      .sort((a, b) => b.smartScore - a.smartScore);

    // ── שלב 4: שמור ושדר ──
    state.wallets = smartWallets.slice(0, 100); // top 100 לדשבורד

    // קטגוריות
    const whales      = smartWallets.filter(w => w.isWhale).length;
    const smartCount  = smartWallets.filter(w => w.isSmart).length;
    const highROI     = smartWallets.filter(w => w.roi > 200).length;

    broadcast('wallets', state.wallets);
    broadcast('wallet_stats', { total: smartWallets.length, whales, smart: smartCount, highROI });

    console.log('[WALLETS] ' + state.wallets.length + ' smart wallets | Whales: ' + whales + ' | High ROI: ' + highROI);
...
async function analyzeWithGroq(text) {
  ...
  try {
    const res = await axios.post('https://api.groq.com/openai/v1/chat/completions', {
      ...
      messages: [{
        role: 'user',
        content: 'Analyze this news for prediction markets. Reply in JSON only: {"impact":"HIGH|MEDIUM|LOW","markets":["m1","m2"],"direction":"YES_UP|NO_UP|NEUTRAL","reason":"brief"} News: ' + text
      }],
      max_tokens: 150,
      temperature: 0.1,
    }, {
...
app.get('/api/health', (_req, res) => res.json({
  ...
  env: {
    news_api:    ENV.NEWS_API_KEY    ? 'set' : 'missing',
    groq_api:    ENV.GROQ_API_KEY    ? 'set' : 'missing',
    telegram:    ENV.TELEGRAM_TOKEN  ? 'set' : 'missing',
    polygon_rpc: ENV.POLYGON_RPC !== 'https://polygon-rpc.com' ? 'custom' : 'default',
    node_env:    ENV.NODE_ENV,
    gamma_url:   ENV.GAMMA_API_URL,
  },
}));
...
server.listen(ENV.PORT, async () => {
  ...
  console.log(` GROQ_API_KEY:   ${ENV.GROQ_API_KEY   ? '✅ set' : '⚠️  optional'}`);
...
.hs-v{font-family:monospace;font-size:13px;font-weight:700;color:var(--ac);}
...
/* CONN BAR */
.conn-bar{padding:5px 22px;font-size:10px;font-family:monospace;display:flex;align-items:center;gap:8px;background:var(--bg2);border-bottom:1px solid var(--br);}
...
.c-val{font-family:monospace;font-size:26px;font-weight:700;color:var(--ac);line-height:1.1;}
...
.t-vol{font-family:monospace;font-size:11px;color:var(--gd);}
...
.al-time{font-family:monospace;font-size:9px;color:var(--tx2);white-space:nowrap;}
/* NEWS */
...
.ocs-v{font-family:monospace;font-size:13px;font-weight:700;}
...
.wst-v{font-family:monospace;font-size:13px;font-weight:700;}
...
<div class="tc" id="tab-wallets">
  ...
  <div class="g4" style="margin-bottom:16px;">
    <div class="card"><div class="c-lbl">👤 ארנקים</div><div class="c-val" id="wlt-total" style="font-size:22px;">—</div></div>
    <div class="card"><div class="c-lbl">🐋 Whales</div><div class="c-val" id="wlt-whales" style="font-size:22px;color:var(--ac2);">—</div></div>
    <div class="card"><div class="c-lbl">🧠 Smart</div><div class="c-val" id="wlt-smart" style="font-size:22px;color:var(--ac);">—</div></div>
    <div class="card"><div class="c-lbl">🔥 ROI 200%+</div><div class="c-val" id="wlt-roi" style="font-size:22px;color:var(--ac3);">—</div></div>
  </div>
  <!-- Filters -->
  <div class="filter-bar" style="margin-bottom:16px;">
    <button class="chip on" onclick="wltFilter(this,'all')">🌐 הכל</button>
    <button class="chip" onclick="wltFilter(this,'whale')">🐋 Whales בלבד</button>
    <button class="chip" onclick="wltFilter(this,'smart')">🧠 Smart Wallets</button>
    <button class="chip" onclick="wltFilter(this,'roi')">🔥 ROI 200%+</button>
    <button class="chip" onclick="wltFilter(this,'winrate')">✅ Win Rate 70%+</button>
    <div style="margin-right:auto;font-size:11px;color:var(--tx2);">ממויין לפי Smart Score ↓</div>
  </div>
  <!-- List -->
  <div style="display:flex;flex-direction:column;gap:10px;" id="wallets-list">
    <div style="text-align:center;padding:40px;color:var(--tx2);">⏳ סורק ארנקים...</div>
  </div>
</div>
...
<div class="status-bar">
  ...
  <span style="font-family:monospace;font-size:9px;" id="clk-disp">--:--:--</span>
...
function renderTopMkts(mkts) {
  ...
  el.innerHTML = mkts.map(function(m) {
    ...
    return '<tr class="new-row"><td class="t-q" title="' + esc(m.question) + '" style="max-width:260px;">' + esc(m.question).slice(0,60) + '...</td>' +
      ...
      '<td style="font-family:monospace;font-weight:700;color:' + (y > 50 ? 'var(--ac)' : 'var(--tx)') + '">' + y + '%</td>' +
...
function renderAllMkts() {
  ...
  el.innerHTML = filtered.map(function(m, i) {
    var y = Math.round((m.yesPrice || .5) * 100);
    return '<tr>' +
      '<td style="font-family:monospace;color:var(--tx2);font-size:10px;">' + (i+1) + '</td>' +
      '<td class="t-q" title="' + esc(m.question) + '" style="max-width:250px;">' + esc(m.question).slice(0,55) + '...</td>' +
      '<td><span class="t-cat">' + (m.category || 'gen') + '</span></td>' +
      '<td style="font-family:monospace;font-weight:700;color:var(--ac)">' + y + '%</td>' +
      '<td style="font-family:monospace;color:var(--dn)">' + (100-y) + '%</td>' +
      '<td><span class="t-vol">' + fmtUSD(m.volume24h || 0) + '</span></td>' +
      '<td><span class="t-vol">' + fmtUSD(m.liquidity  || 0) + '</span></td>' +
      '<td>' + sigBadge(y) + '</td>' +
      '<td><button class="btn btn-p" style="font-size:10px;padding:4px 8px;" data-url="' + m.url + '" onclick="openUrl(this)">פתח</button></td>' +
    '</tr>';
  }).join('');
}
...
function whaleTxHTML(tx, isNew) {
  ...
  return '<div class="wtx' + (isNew ? ' new-row' : '') + '">' +
    ...
    '<div style="flex:1;">' +
      ...
      '<div style="font-family:monospace;font-size:9px;color:var(--tx2);">' + (tx.walletShort || '0x...') + '</div>' +
    '</div>' +
    '<div style="text-align:left;">' +
      '<div style="font-family:monospace;font-size:12px;font-weight:700;color:var(--gd);">' + (tx.amountFormatted || fmtUSD(tx.amount || 0)) + '</div>' +
...
function onOpps(opps) {
  ...
  el.innerHTML = opps.map(function(o) {
    ...
    return '<div class="opp-card ' + tp + '">' +
      ...
      '</div>' +
      '<button class="btn btn-p" style="width:100%;margin-top:10px;font-size:10px;" data-url="' + m.url + '" onclick="openUrl(this)">📊 פתח ב-Polymarket</button>' +
    '</div>';
  }).join('');
}
...
function onWallets(wallets) {
  ...
  // עדכן stats bar
  var whales  = wallets.filter(function(w){return w.isWhale;}).length;
  var smart   = wallets.filter(function(w){return w.isSmart;}).length;
  var highROI = wallets.filter(function(w){return (w.roi||0)>200;}).length;
  setText('wlt-total',  wallets.length + '');
  setText('wlt-whales', whales + '');
  setText('wlt-smart',  smart + '');
  setText('wlt-roi',    highROI + '');

  // פילטר נוכחי
  var filt = S.walletFilter || 'all';
  var filtered = wallets.filter(function(w) {
    if (filt === 'whale')  return w.isWhale;
    if (filt === 'smart')  return w.isSmart;
    if (filt === 'roi')    return (w.roi||0) > 200;
    if (filt === 'winrate')return (w.winRate||0) > 70;
    return true;
  });

  if (!filtered.length) {
    el.innerHTML = '<div style="text-align:center;padding:40px;color:var(--tx2);">אין ארנקים התואמים את הפילטר</div>';
    return;
  }

  el.innerHTML = filtered.map(function(w, i) {
    var roiColor = (w.roi||0)>500?'var(--ac3)':(w.roi||0)>200?'var(--gd)':(w.roi||0)>100?'var(--ac)':'var(--tx)';
    var wrColor  = (w.winRate||0)>80?'var(--ac)':(w.winRate||0)>60?'var(--gd)':'var(--tx2)';
    var badges = '';
    if (w.isWhale) badges += '<span style="background:rgba(0,212,255,.15);border:1px solid rgba(0,212,255,.3);color:var(--ac2);font-size:9px;padding:2px 6px;border-radius:4px;font-weight:700;">🐋 WHALE</span> ';
    if (w.isSmart) badges += '<span style="background:rgba(0,255,136,.1);border:1px solid rgba(0,255,136,.3);color:var(--ac);font-size:9px;padding:2px 6px;border-radius:4px;font-weight:700;">🧠 SMART</span> ';
    if ((w.roi||0)>500) badges += '<span style="background:rgba(255,107,53,.15);border:1px solid rgba(255,107,53,.4);color:var(--ac3);font-size:9px;padding:2px 6px;border-radius:4px;font-weight:700;">🔥 HIGH ROI</span>';
    var streakHtml = (w.streak && w.streak > 2) ?
      '<span style="font-size:9px;color:var(--ac3);font-weight:700;">🔥 ' + w.streak + ' ברצף</span>' : '';
    return '<div class="wlt-card">' +
      '<div style="display:flex;align-items:center;justify-content:space-between;gap:6px;flex-wrap:wrap;">' +
        '<div style="display:flex;align-items:center;gap:6px;">' +
          '<span style="font-family:monospace;font-size:11px;color:var(--ac2);">' + esc(w.shortAddress || '0x...') + '</span>' +
          (w.name ? '<span style="font-size:10px;color:var(--tx2);">(' + esc(w.name) + ')</span>' : '') +
        '</div>' +
        '<div style="display:flex;align-items:center;gap:4px;">' +
          ...
          '<span style="font-family:monospace;font-size:10px;color:var(--ac3);font-weight:700;">' + (w.smartScore||0) + ' pts</span>' +
        '</div>' +
      '</div>' +
      (badges ? '<div style="margin-top:5px;display:flex;gap:4px;flex-wrap:wrap;">' + badges + '</div>' : '') +
      '<div class="wlt-stats">' +
        '<div class="wst"><div class="wst-v" style="color:' + roiColor + ';">' + (w.roi > 0 ? '+' : '') + (w.roi||0) + '%</div><div class="wst-l">ROI</div></div>' +
        '<div class="wst"><div class="wst-v" style="color:' + wrColor + ';">' + (w.winRate||0) + '%</div><div class="wst-l">Win Rate</div></div>' +
        '<div class="wst"><div class="wst-v" style="color:var(--gd);">' + fmtUSD(w.profit||0) + '</div><div class="wst-l">רווח</div></div>' +
        '<div class="wst"><div class="wst-v">' + (w.trades||0) + '</div><div class="wst-l">עסקאות</div></div>' +
        '<div class="wst"><div class="wst-v">' + fmtUSD(w.volume||0) + '</div><div class="wst-l">נפח</div></div>' +
        '<div class="wst"><div class="wst-v" style="color:var(--ac2);">' + fmtUSD(w.avgPosition||0) + '</div><div class="wst-l">פוז' + "'" + 'ה ממוצעת</div></div>' +
      '</div>' +
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-top:4px;">' +
        streakHtml +
        '<a href="https://polymarket.com/profile/' + w.address + '" target="_blank" rel="noopener"><button class="btn btn-o" style="font-size:10px;padding:4px 9px;">📊 Polymarket</button></a>' +
      '</div>' +
    '</div>';
  }).join('');
}
...
// ── WALLET FILTER ─────────────────────────────────
function wltFilter(el, filt) {
  var bar = el.closest('.filter-bar') || el.parentElement;
  bar.querySelectorAll('.chip').forEach(function(c) { c.classList.remove('on'); });
  el.classList.add('on');
  S.walletFilter = filt;
  if (S.wallets && S.wallets.length) onWallets(S.wallets);
}

// ── OPEN URL HELPER ──────────────────────────────
function openUrl(el) {
  var url = (typeof el === 'string') ? el : (el.getAttribute ? el.getAttribute('data-url') : '');
...
.conn-m{padding:4px 16px;font-size:10px;font-family:monospace;display:flex;align-items:center;gap:6px;background:var(--bg2);border-bottom:1px solid var(--br);flex-shrink:0;}
...
.mc-v{font-family:monospace;font-size:21px;font-weight:700;line-height:1;}
...
.pnum{font-family:monospace;font-size:13px;font-weight:700;}
.mvol{font-family:monospace;font-size:11px;color:var(--gd);}
...
.wi-addr{font-family:monospace;font-size:10px;color:var(--tx2);margin-top:2px;}
.wi-mkt{font-size:10px;color:var(--ac2);margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
.wi-amt{font-family:monospace;font-size:14px;font-weight:700;color:var(--gd);}
...
.al-tm{font-family:monospace;font-size:9px;color:var(--tx2);}
...
.ocs-v{font-family:monospace;font-size:14px;font-weight:700;}
.ocs-l{font-size:8px;color:var(--tx2);margin-top:2px;text-transform:uppercase;}
...
<div class="scroll-a" id="m-scroll">
  ...
  <div class="page" id="m-page-whales">
    ...
    <div style="font-size:11px;font-weight:700;color:var(--tx2);margin-bottom:10px;font-family:monospace;">📡 REAL-TIME FEED</div>
    <div id="m-wf"></div>
...
function mMktCard(m) {
  ...
  return '<div class="mkt-item new-row" data-url="'+mEsc(m.url)+'" onclick="openUrl(this)">'+
...
function mOnOpps(opps){
  ...
  el.innerHTML=opps.map(function(o){
    ...
    return '<div class="oc '+tp+'">'+
      ...
      '<button class="oc-btn" data-url="'+(m.url||'https://polymarket.com')+'" onclick="openUrl(this)">📊 פתח ב-Polymarket</button>' +
      '<div class="oc-stats">'+
        '<div class="ocs"><div class="ocs-v" style="color:var(--ac)">'+y+'%</div><div class="ocs-l">הסתברות</div></div>'+
        '<div class="ocs"><div class="ocs-v" style="color:var(--gd)">'+(o.expectedValue||'?')+'</div><div class="ocs-l">EV</div></div>'+
        '<div class="ocs"><div class="ocs-v" style="color:var(--ac2)">'+(o.score||0)+'</div><div class="ocs-l">Score</div></div>'+
